    ...
    <div className="min-h-screen pt-24 bg-background pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6 text-left">
            ...
            <div className="pt-2">
              <div className="p-5 border-l-2 border-primary bg-primary/5 rounded-r-xl">
                <p className="text-xs sm:text-sm font-semibold tracking-tight text-white font-display">
          ...
          <div className="lg:col-span-5 bg-neutralDark/40 border border-zinc-800 rounded-3xl p-6 md:p-8 space-y-4 text-center">
            <Rocket className="w-10 h-10 text-primary mx-auto" />
            <h3 className="text-lg font-bold text-white font-display">Why Autovate?</h3>
            ...
            <div className="text-[10px] font-mono text-zinc-500 pt-2 border-t border-zinc-850">
        ...
        <section className="space-y-6 text-left">
          <div className="space-y-2">
            <h3 className="text-xl sm:text-2xl font-bold font-display text-white">Bot vs Agency vs VA</h3>
            <p className="text-xs sm:text-sm text-zinc-500">Compare the metrics of templates against traditional agency retainer hires or manual VAs.</p>
          </div>

          <div className="overflow-x-auto no-scrollbar border border-zinc-850 rounded-2xl bg-card">
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className="border-b border-zinc-800 text-zinc-400 font-mono text-xs uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-zinc-800/80 text-xs sm:text-sm text-zinc-300">
                {compTable.map((row, idx) => (
                  <tr key={idx} className="hover:bg-zinc-900/30 transition-colors">
                    ...
                    <td className="p-4 text-zinc-400">{row.agency}</td>
                    <td className="p-4 pr-6 text-zinc-400">{row.va}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="space-y-6 text-center max-w-4xl mx-auto">
          <div className="space-y-2">
            <h3 className="text-xl sm:text-2xl font-bold font-display text-white">Our Technology Ecosystem</h3>
            <p className="text-xs sm:text-sm text-zinc-500">We build on industry-leading, secure, visual cloud platforms.</p>
          </div>

          <div className="flex flex-wrap justify-center gap-4">
            {['Make.com', 'Zapier', 'n8n.io', 'OpenAI API', 'Bland.ai', 'HubSpot API', 'Stripe Payments', 'WhatsApp API'].map((tech) => (
              <span key={tech} className="px-5 py-3 bg-zinc-900 border border-zinc-800 rounded-xl font-mono text-xs text-zinc-400 uppercase tracking-wider font-semibold hover:border-primary/20 hover:text-white transition-all cursor-default">
                {tech}
              </span>
            ))}
          </div>
        ...
        <section className="space-y-8 text-left">
          <div className="space-y-2">
            <h3 className="text-xl sm:text-2xl font-bold font-display text-white">Our Solution Architects</h3>
            <p className="text-xs sm:text-sm text-zinc-500">The automation engineers behind the Autovate templates catalog.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {team.map((t) => (
              <div key={t.name} className="bg-card border border-zinc-900 rounded-2xl p-6 space-y-4 group hover:border-primary/10 transition-colors">
                <img
                  src={t.img}
                  alt={t.name}
                ...
                <div className="space-y-1">
                  <h4 className="text-base font-bold text-white font-display group-hover:text-primary transition-colors">{t.name}</h4>
                  <p className="text-[10px] font-mono text-zinc-550 text-zinc-500 uppercase">{t.role}</p>
                </div>
                <p className="text-zinc-400 text-xs leading-relaxed">{t.bio}</p>
              </div>
            ))}
          </div>
        </section>
